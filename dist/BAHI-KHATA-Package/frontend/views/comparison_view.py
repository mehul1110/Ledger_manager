...
def show_comparison_view(app):
    ...
    month1_entry = DateEntry(select_frame, width=10, background='darkblue', foreground='white', borderwidth=2, date_pattern='mm-yyyy')
    month1_entry.grid(row=0, column=1, padx=5)
    month1_entry.set_date(None)

    tk.Label(select_frame, text="Month 2 (MM-YYYY):", font=label_font, bg='white').grid(row=0, column=2, padx=5)
    month2_entry = DateEntry(select_frame, width=10, background='darkblue', foreground='white', borderwidth=2, date_pattern='mm-yyyy')
    month2_entry.grid(row=0, column=3, padx=5)
    month2_entry.set_date(None)
    ...
    def apply_comparison():
        ...
        try:
            ...
            d1 = datetime.strptime(month1_entry.get(), '%m-%Y')
            d2 = datetime.strptime(month2_entry.get(), '%m-%Y')
