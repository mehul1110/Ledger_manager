...
def show_add_payment_form(app, go_back_callback):
    ...
    other_account_label = tk.Label(app.root, text="Other Account (for Fund lend):", font=label_font)
    other_account_entry = tk.Entry(app.root, font=entry_font)
    ...
    def hide_all_extras():
        for w in [author_label, author_entry, item_label, item_entry, desc_radio1, desc_radio2, type_label, type_menu, fd_duration_label, fd_duration_number, fd_duration_unit_menu, fd_interest_label, fd_interest_entry, other_account_label, other_account_entry]:
            w.grid_forget()
    def show_extras(*args):
        hide_all_extras()
        row = 6
        if narration_var.get() == "Article appreciation amount":
            author_label.grid(row=row, column=0, padx=10, pady=10, sticky='e')
            author_entry.grid(row=row, column=1, padx=10, pady=10, sticky='ew')
            row += 1
        ...
        if narration_var.get() == "FD in bank":
            ...
            row += 1
        if narration_var.get() == "Fund lend to other accounts":
            other_account_label.grid(row=row, column=0, padx=10, pady=10, sticky='e')
            other_account_entry.grid(row=row, column=1, padx=10, pady=10, sticky='ew')
    ...
    for e in [account_menu, amount_entry, cheque_entry, date_entry, author_entry, item_entry, fd_duration_number, fd_interest_entry, other_account_entry]:
        style_entry(e)
    ...
    if narration_var.get() == "Article appreciation amount":
        fields.append(("Author", author_entry, author_entry))
    if narration_var.get() == "Property":
        fields.extend([
    ...
    if narration_var.get() == "Fund lend to other accounts":
        fields.append(("Other Account", other_account_entry, other_account_entry))
    ...
    def submit(event=None):
        ...
        try:
            add_payment(
                ...
                author=author_entry.get() if narration_var.get() == 'Article appreciation amount' else None,
                ...
                remarks=other_account_entry.get() if narration_var.get() == 'Fund lend to other accounts' else None
            )
    ...
    narration_menu.bind('<Return>', lambda e: (
        author_entry.focus_set() if narration_var.get() == 'Article appreciation amount' else
        item_entry.focus_set() if narration_var.get() == 'Property' else
        fd_duration_number.focus_set() if narration_var.get() == 'FD in bank' else
        other_account_entry.focus_set() if narration_var.get() == 'Fund lend to other accounts' else
    ...
    other_account_entry.bind('<Return>', lambda e: date_entry.focus_set())
