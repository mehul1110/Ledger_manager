...
def show_receipts_view(app):
    ...
    tk.Label(app.root, text="Receipts", font=label_font, bg="white").pack(pady=20)
    ...
    try:
        ...
        cursor_filters.execute("SELECT DISTINCT account_name FROM receipts ORDER BY account_name")
        ...
        cursor_filters.execute("SELECT DISTINCT narration FROM receipts ORDER BY narration")
    ...
    columns = ["Receipt ID", "Account Name", "Amount", "Mode", "Narration", "Remarks", "Date"]
    ...
    def apply_filters():
        ...
        # Construct query
        query = (
            "SELECT receipt_id, account_name, amount, mop, narration, remarks, date "
        ...
        query += " ORDER BY receipt_id ASC"
        print(f"[DEBUG] receipts_view executing: {query}")
        try:
            conn_data = db_connect.get_connection()
            cursor_data = conn_data.cursor()
            cursor_data.execute(query)
            rows = cursor_data.fetchall()
            print(f"[DEBUG] receipts_view fetched {len(rows)} rows")
            for row in rows:
                tree.insert('', 'end', values=row)
        except Exception as e:
            messagebox.showerror('Database Error', f'Failed to fetch receipts: {e}')
        finally:
            cursor_data.close()
            conn_data.close()
    ...
    def export_to_csv():
        ...
        try:
            ...
            messagebox.showinfo('Export Successful', f'Receipts exported to {file_path}')
        except Exception as e:
